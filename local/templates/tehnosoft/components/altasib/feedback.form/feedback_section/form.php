<div class="afbf_feedback_poles">
	...
	<?endif;?>
	<form class="form" id="f_feedback_<?=$ALX?>" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
...
?>
				<div id="afbf_<?=mb_strtolower($arField["CODE"])?>" class="form-group afbf_item-pole<?=$fieldClass?>">
...
<?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
							...
							<input type="text" size="40" placeholder="<?=$arField["NAME"]?>" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="<?=trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"]))?>" class="afbf_input_text form-control" />
<?						else:?>
							<input type="text" size="40" placeholder="<?=$arField["NAME"]?>" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext form-control" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" />
<?						endif;?>
...
<?		echo bitrix_sessid_post()?>
		<div class="afbf_submit_block">
			<input type="submit" class="fb_close afbf-btn btn" id="fb_close_<?=$ALX?>" name="SEND_FORM" value="<?=GetMessage('ALX_TP_MESSAGE_SUBMIT')?>" />
		</div>
	</form>
